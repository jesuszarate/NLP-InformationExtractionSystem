...
def getTarget(ne_tree):
    for i in range(0, len(ne_tree)):
        ...
        m = re.findall(r'(' + '\\bexploded\\b|\\battack\\b|\\bterrorist\\b|\\bbomb\\b|\\battacked\\b|\\bblown\\b|\\bfire\\b' + ')', item)

        if len(m) > 0:

            if m[0] == 'attack' or m[0] == 'terrorist' or m[0] == 'bomb':
                for aPos in range(i, i + 10):
                    if aPos < len(ne_tree):
                        m = re.findall(r'(' + '\\bagainst\\b|\\bon\\b|\\battack\\b|\\bthrown\\b' + ')', str(ne_tree[aPos]).lower())
                        if len(m) > 0:
